               /* School Billing Management System
                  ...
                  * Compiler: C++ 17
                  Limitations
                     ...
                     User has no payment method in program
               */
#include<iostream>
#include<string.h>
#include <vector>
...
void new_reg(){
      ...
      for (int i = stud_count;i < stud_count + num;i++){
          ...
          student.pay_date.mm = student.reg_date.mm + 1;  //payment is 1 month after registration
          student.pay_date.yy = student.reg_date.yy;
...
void reorder(){   // orders records by name
    for (int i{};i < stud_count;i++){
        for (int j{};j < stud_count - 1;j++){
            if (strcmp(stud[j + 1].name, stud[j].name) <= 1 || stud[j].name[0] == '\0'){
